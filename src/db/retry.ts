 ...
 *  - pg's CLIENT-level `connectionTimeoutMillis` expiry ("timeout expired")
 *  - pg's POOL-level acquire timeout ("timeout exceeded when trying to connect",
 *    no `.code`, so only the message branch can catch it)
 *

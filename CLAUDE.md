...
- **Thread expansion.** `src/services/conversation.ts` matches the address on the typed silver fields (`lower(from_email)` or `to_emails ? <address>`, both indexed), then returns EVERY message of the matched threads — so the owner's own answer belongs to the exchange even when the prospect is only a Cc on it. If silver matches nothing, one bounded `payload::text ILIKE` pass over bronze catches the Cc-only / not-yet-parsed case, which keeps "nobody has this exchange" an honest answer.

...
Bronze stays the source of truth; silver is a rebuildable view. Contact silver columns: `display_name, primary_email, emails[], phones[], organization, job_title, photo_url, updated_at, deleted`. Message silver columns: `from_email, from_name, to_emails[], cc_emails[], subject, snippet, sent_at, labels[], history_id`.
...
The mirror holds exchanges the outreach provider never saw — a forwarding rule on the sending mailbox drains the reply before the provider reads it, so the prospect's answer and everything after it exist ONLY here. This endpoint is what makes those readable as a conversation with a named person.

- **A read, never a fetch.** Zero Google calls per page view, and nothing new is ingested. `src/services/message-body.ts` derives the readable body ON READ from the bronze `payload` (MIME walk, `text/plain` preferred, `text/html` reduced to text when it is the only part). Deriving on read rather than at ingest was deliberate: every one of the ~700k historical messages is readable immediately, with no backfill of that table, no boot-window work and no schema change. Conversation reads are bounded to one person's exchange, so the parse is paid on a handful of rows.
- **Thread expansion, entirely from indexes.** `src/services/conversation.ts` matches the address on the typed silver fields — `lower(from_email)` as sender, GIN over `to_emails` and `cc_emails` as recipient — then returns EVERY message of the matched threads, so the owner's own answer belongs to the exchange even when the prospect is only a Cc on it. **There is deliberately NO `payload::text ILIKE` fallback over bronze**: measured on the real mirror (~660k messages) that scan took 75s and it ran on the MOST common question — an address we have never mailed — making the honest "nobody has this exchange" answer the slowest one in the endpoint (the indexed path answers in ~1s). Silver is written at ingest and rebuilt for every bronze row on boot, so From/To/Cc coverage is complete without it; a new participant field must therefore become a silver COLUMN, never a payload scan.
